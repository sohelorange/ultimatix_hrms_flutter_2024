...
class LiveTrackingController extends GetxController {
  ...
  Rx<LocationTrackResponse> locationTrackingResponse =
      LocationTrackResponse().obs;
  String selectedDate =
    ...
    try {
      ...
      if (address.isNotEmpty) {
        ...
        isolateData.answerPort.send(
            "${place.name}, ${place.street}, ${place.subLocality}, ${place.thoroughfare}, ${place.locality}, ${place.administrativeArea}, ${place.country}, ${place.postalCode} + ${geoLocation.latitude},${geoLocation.longitude}");
      }
    } catch (e) {
      e.printError();
    }
  ...
  Future<void> showTrack(LocationTrackResponse value) async {
    ...
    for (int i = 0; i < value.data!.length; i++) {
      locList.add([
        value.data!.elementAt(0).longitude!.toDouble(),
        value.data!.elementAt(0).latitude!.toDouble()
      ]);
    }
  ...
  Future<void> calculateTimeDifference(String time1Str, String time2Str) async {
    ...
    double differenceInHours =
        difference.inHours.toDouble() + (difference.inMinutes % 60) / 60.0;
  ...
  void setInGeoFencingLocation() async {
    ...
    await webController?.runJavaScript(
        'geoFencing(72.502564, 23.011767, 72.502564, 23.011767)');
  }

  void setOutGeoFencingLocation() async {
    webController?.runJavaScript('displayLocation(72.20294, 22.46072);');
    await webController
        ?.runJavaScript('geoFencing(72.502564, 23.011767, 72.20294, 22.46072)');
  }
